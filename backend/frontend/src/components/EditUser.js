...
const EditUser = () => {
  ...
  const [tipe, settipe] = useState("Personal"); // Set default tipe yang lebih sesuai konteks notes
  ...
  const updateUser = async (e) => {
    ...
    try {
      ...
      navigate("/userlist"); // Arahkan kembali ke daftar catatan setelah pembaruan
    } catch (error) {
      console.error("Error updating note:", error);
      // Tambahkan penanganan error UI jika diperlukan (misalnya, menampilkan pesan error)
    }
  };
  ...
  const getUserById = async () => {
    try {
      ...
      // Tambahkan penanganan error UI jika diperlukan (misalnya, menampilkan pesan error atau kembali ke daftar)
    }
  };
